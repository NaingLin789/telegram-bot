...
last_session_notify = {"time": 0}  # Session expire notify cooldown
    ...
    while True:
        try:
            ...
            if "login" in r.url.lower() or r.status_code != 200:
                now = time.time()
                if now - last_session_notify["time"] > 21600:  # 6 နာရီတစ်ကြိမ်ပဲ notify
                    last_session_notify["time"] = now
                    notify_admin("⚠️ *Batman SESSION expire ဖြစ်နေပြီ!*\n\n`/setsession VALUE` ပို့ပါ။")
            else:
...
# BROADCAST PHOTO
# ==============================
def do_broadcast_photo(photo_id, caption):
    total = len(known_users)
    success = 0
    fail = 0
    for uid in list(known_users):
        try:
            bot.send_photo(uid, photo_id, caption=caption, parse_mode="Markdown")
            success += 1
            time.sleep(0.05)
        except:
            fail += 1
    notify_admin(
        f"📢 *Broadcast ပုံ ပြီး*\n\n"
        f"✅ အောင်မြင် — {success} ဦး\n"
        f"❌ မပို့ရ — {fail} ဦး\n"
        f"👥 စုစုပေါင်း — {total} ဦး"
    )

pending_broadcast_photo = {}
...
def admin_broadcast_photo(message):
    ...
    pending_broadcast_photo[message.chat.id] = {"caption": caption}
    bot.send_message(message.chat.id,
        "📸 ယခု ပုံကို ပို့ပါ — Customer အားလုံးဆီ broadcast လုပ်မယ်",
        parse_mode="Markdown")

@bot.message_handler(content_types=["photo"], func=lambda m: str(m.chat.id) == ADMIN_CHAT_ID and m.chat.id in pending_broadcast_photo)
def admin_broadcast_photo_handler(message):
    if message.chat.id not in pending_broadcast_photo:
        return
    data = pending_broadcast_photo.pop(message.chat.id)
    caption = data.get("caption", "")
    photo_id = message.photo[-1].file_id
    total = len(known_users)
    bot.send_message(message.chat.id,
        f"📢 *{total} ဦး* ဆီ ပုံ ပို့နေပါသည်...",
        parse_mode="Markdown")
    thread = threading.Thread(target=do_broadcast_photo, args=(photo_id, caption))
    thread.daemon = True
    thread.start()
...
def admin_reject(message):
    ...
    try:
        ...
        bot.send_message(customer_uid,
            f"❌ *မအောင်မြင်ပါ*\n\n🆔 `{order_id}`\n\nပြန်စစ်ဆေးပြီး Admin ဆက်သွယ်ပါ။\n\n⚠️ ငွေလွှဲ SS အတုဆို မအောင်မြင်နိုင်ပါခင်ဗျ\n\n📞 09 940 940 010",
            parse_mode="Markdown", reply_markup=get_main_menu())
...
def admin_send_photo(message):
    """Admin က customer ကို ပုံ + caption ပို့မယ် — /photo ORD1001 caption"""
    if str(message.chat.id) != ADMIN_CHAT_ID:
        return
    try:
        parts = message.text.split(" ", 2)
        if len(parts) < 2:
            bot.send_message(message.chat.id,
                "📸 *ပုံပို့နည်း*\n\n"
                "1️⃣ ဒီ command ပို့ပါ:\n`/photo ORD1001 caption text`\n\n"
                "2️⃣ ပြီးရင် ပုံ reply လုပ်ပြီး ပို့ပါ",
                parse_mode="Markdown")
            return
        order_id = parts[1].upper()
        ...
        if order_id not in order_by_id:
            bot.send_message(message.chat.id, f"❌ `{order_id}` မတွေ့ပါ", parse_mode="Markdown")
            return
        # Store pending photo request
        pending_sendss[f"photo_{message.chat.id}"] = {
            "order_id": order_id,
            "caption": caption,
            "customer_uid": order_by_id[order_id]
        }
        bot.send_message(message.chat.id,
            f"📸 ယခု ပုံကို reply လုပ်ပြီး ပို့ပါ — `{order_id}` ထံ ရောက်မယ်",
            parse_mode="Markdown")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ {str(e)}")

@bot.message_handler(content_types=["photo"], func=lambda m: str(m.chat.id) == ADMIN_CHAT_ID and f"photo_{m.chat.id}" in pending_sendss)
def admin_send_photo_handler(message):
    """Admin ပို့တဲ့ ပုံကို customer ဆီ forward လုပ်မယ်"""
    try:
        key = f"photo_{message.chat.id}"
        if key not in pending_sendss:
            return
        data = pending_sendss.pop(key)
        customer_uid = data["customer_uid"]
        order_id = data["order_id"]
        caption = data["caption"]
        photo_id = message.photo[-1].file_id
        full_caption = f"📸 *{order_id}*\n\n{caption}\n\nကျေးဇူးတင်ပါတယ် 🙏" if caption else f"📸 *{order_id}*\n\nကျေးဇူးတင်ပါတယ် 🙏"
        bot.send_photo(customer_uid, photo_id, caption=full_caption, parse_mode="Markdown", reply_markup=get_main_menu())
        bot.send_message(message.chat.id, f"✅ ပုံ ပို့ပြီး — `{order_id}`", parse_mode="Markdown")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ {str(e)}")
