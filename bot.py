...
def batman_add_player(customer_name, score):
    try:
        ...
        # Generate username from form or get auto-generated one
        username_input = soup.find('input', {'id': 'username'}) or soup.find('input', {'name': 'username'})
        ...
        r2 = session.post(f"{BATMAN_URL}/account/player/add",
        ...
        if r2.status_code == 200:
            try:
                json_data = r2.json()
                username = json_data.get('username') or json_data.get('login_id') or json_data.get('account_id', '')
                if username:
                    return {"success": True, "username": username, "password": password,
                            "score": score, "url": "http://m.batman688.com"}
            except:
                soup2 = BeautifulSoup(r2.text, 'html.parser')
                # Try to find username in response HTML
                username_el = soup2.find('td', text=lambda t: t and t.startswith('B'))
                if username_el:
                ...
                if 'success' in r2.text.lower() or 'player' in r2.text.lower():
                    return {"success": True, "username": "Portal မှာ စစ်ပါ",
                            "password": password, "score": score, "url": "http://m.batman688.com"}
...
def batman_deposit(player_id, amount):
    try:
        ...
        r2 = session.post(f"{BATMAN_URL}/account/player/deposit",
                         data=data, timeout=15, allow_redirects=True)
...
# ==============================
# AUTO PROCESS (Admin Confirm ပြီးမှ)
# ==============================
...
# ==============================
# ADMIN — /confirm (Slip စစ်ပြီး Auto လုပ်)
# ==============================
